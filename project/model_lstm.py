...
class NMTmodel(torch.nn.Module):
    ...
    def __init__(self, encoder, decoder, sourceWord2ind, targetWord2ind, startToken, unkToken, padToken, endToken):
        ...
        self.targetInd2word = {i: w for w, i in self.targetWord2ind.items()}
    ...
    def forward(self, source, target, teacher_forcing_ratio = 0.5):
        ...
        encoder_outputs, (h_n, c_n) = self.encoder(source_padded, source_lengths)

        input = target_padded[0, :]
        mask = self.create_mask(source_padded)
        
        for t in range(1, target_len):
            output, (h_n, c_n), _ = self.decoder(input, h_n, c_n, encoder_outputs, mask)
    ...
    def translateSentence(self, sentence, limit=1000):
        ...
        with torch.no_grad():
            encoder_outputs, (h_n, c_n) = self.encoder(source, [len(source)])
            ...
            for i in range(limit):
                ...
                output, (h_n, c_n), _ = self.decoder(target, h_n, c_n, encoder_outputs, mask)
        ...
        return [self.targetInd2word[i] for i in result[1:]]
...
class Encoder(torch.nn.Module):
    def __init__(self, embed_size, enc_hid_size, dec_hid_size, source_size, dropout):
        ...
        self.rnn = torch.nn.LSTM(embed_size, enc_hid_size, bidirectional = True)

    def forward(self, source_padded, source_lengths):
        E = self.dropout(self.embed(source_padded))

        outputPacked, (h_n, c_n) = self.rnn(torch.nn.utils.rnn.pack_padded_sequence(E, source_lengths, enforce_sorted=False))
        output, _ = torch.nn.utils.rnn.pad_packed_sequence(outputPacked)

        h_n = torch.cat((h_n[-2,:,:], h_n[-1,:,:]), dim = 1)
        c_n = torch.cat((c_n[-2,:,:], c_n[-1,:,:]), dim = 1)

        h_n = torch.nn.functional.relu(self.projection(h_n))
        c_n = torch.nn.functional.relu(self.projection(c_n))
...
class Decoder(torch.nn.Module):
    def __init__(self, embed_size, enc_hid_size, dec_hid_size, target_size, dropout):
        ...
        self.rnn = torch.nn.LSTM((enc_hid_size * 2) + embed_size, dec_hid_size)
        self.projection = torch.nn.Linear((enc_hid_size *  2) + dec_hid_size + embed_size, target_size)

    def forward(self, source, h_n, c_n, encoder_outputs, mask):
        source = source.unsqueeze(0)
        attn = self.attention(h_n, encoder_outputs, mask).unsqueeze(1)
        ...
        output, (h_n, c_n) = self.rnn(t, (h_n.unsqueeze(0), c_n.unsqueeze(0)))
        ...
        return Z, (h_n.squeeze(0), c_n.squeeze(0)), attn.squeeze(1)
