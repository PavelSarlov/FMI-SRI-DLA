...
if len(sys.argv)>1:
    if sys.argv[1] == '3':
        ...
        try:
            ...
            # print(J, du_w.shape, dVt.shape, dv.shape)
        ...
        try:
            [u_w,Vt,v,J,du_w,dVt,dv] = np.load('test5.npy',allow_pickle=True)
            J1, du_w1, dVt1, dv1 = grads.lossAndGradientBatched(u_w, Vt, v)
            assert du_w.shape == du_w1.shape, "Формата на du_w не съвпада с очакваната"
            assert dVt.shape == dVt1.shape, "Формата на dVt не съвпада с очакваната"
            assert np.max(np.abs(J-J1))<1e-3, "Стойноста на J не съвпадат с очакваната"
            assert np.max(np.abs(du_w-du_w1))<1e-3, "Стойностите на du_w не съвпадат с очакваните"
        ...
        try:
            [data,U0,V0,v0,U,V,v] = np.load('test6.npy',allow_pickle=True)
            with open ('test3', 'rb') as fp: freqs = pickle.load(fp)
            contextFunction = lambda c: [c,4543,6534,12345,9321,1234]
            U1,V1,v1 = w2v_sgd.stochasticGradientDescend(data,np.copy(U0),np.copy(V0),np.copy(v0),contextFunction,grads.lossAndGradientCumulative)
            assert U.shape == U1.shape, "Формата на U не съвпада с очакваната"
            assert V.shape == V1.shape, "Формата на V не съвпада с очакваната"
            assert v.shape == v1.shape, "Формата на v не съвпада с очакваната"
            assert np.max(np.abs(U-U1))<1e-3, "Стойностите на U не съвпадат с очакваните"
            assert np.max(np.abs(V-V1))<1e-3, "Стойностите на V не съвпадат с очакваните"
            assert np.max(np.abs(v-v1))<1e-3, "Стойностите на V не съвпадат с очакваните"
